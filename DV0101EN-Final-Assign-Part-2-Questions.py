...
app.layout = html.Div([
    ...
    # Combine both dropdowns in one row
    html.Div([
        html.Div([
...
def update_output_container(report_type, selected_year):
    if report_type == 'Recession Period Statistics':
        ...
        # Plot 1: Line chart for average automobile sales during recession
        yearly_rec = recession_data.groupby('Year')['Automobile_Sales'].mean().reset_index()
        R_chart1 = dcc.Graph(figure=px.line(yearly_rec, x='Year', y='Automobile_Sales',
                                            title="Average Automobile Sales Fluctuation Over Recession Period"))

        # Plot 2: Bar chart average vehicle sales by type
        ...
        # Plot 4: Bar chart for effect of unemployment on sales
        ...
        # Plot 1: Line chart for average sales over all years
        yas = data.groupby('Year')['Automobile_Sales'].mean().reset_index()
        Y_chart1 = dcc.Graph(figure=px.line(yas, x='Year', y='Automobile_Sales',
                                            title='Average Automobile Sales Over Years'))

        # Plot 2: Line chart total monthly sales for selected year
        ...
        # Plot 3: Bar chart average sales by vehicle type
        avr_vdata = yearly_data.groupby('Vehicle_Type')['Automobile_Sales'].mean().reset_index()
        Y_chart3 = dcc.Graph(figure=px.bar(avr_vdata, x='Vehicle_Type', y='Automobile_Sales',
                                           title='Average Vehicle Sales by Type in {}'.format(selected_year)))

        # Plot 4: Pie chart for advertisement expenditure by vehicle type
